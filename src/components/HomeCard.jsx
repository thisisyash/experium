...
import { Card, CardContent, Typography, CardMedia, Collapse } from '@mui/material';
...
const CardComponent = () => {
  ...
  // Cards data
  const cards = [
    { id:1,tagId: "CactusGarden", title: 'Cactus Garden', videoUrl: 'http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4' },
    { id:2,tagId: "PalmGarden", title: 'Palm Garden', videoUrl: 'http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4' },
    { id:3,tagId: "Amphitheatre", title: 'Amphitheatre', videoUrl: 'http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4' },
    { id:4,tagId: "Sculpture", title: "Sculpture's", videoUrl: 'http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4' }
  ];
  ...
  return (
    <Grid container spacing={2} justifyContent="center">
      {cards.map((card, index) => {
        return (
          <Grid item xs={12} sm={6} md={4} lg={4} key={card.id } sx={{ background: "#f5f2f5" }}>
            <div ref={(el) => (cardRefs.current[card.id] = el)} id={card.tagId}>
              {/* Apply Grow animation only when card is visible */}
                ...
                <Card sx={{
                  ...
                  height: "85vh",
                  boxShadow: 'none',
                  flexWrap: 'wrap', // Ensure content wraps in smaller screens
                  marginTop: "500px",
                  background:'#f5f2f5'
                }}>
                  {/* Video Animation: Move from bottom */}
                  <CardMedia
                    component="video"
                    sx={{
                      width: { xs: '100%', sm: '100%', lg:'50%' }, // Full width on extra small screens, 60% on larger ones
                      height:'auto', // Auto height on smaller screens, 70vh on larger ones
                      objectFit: 'cover',
                      borderTopLeftRadius: 2,
                      borderBottomLeftRadius: 2,
                      background:'#f5f2f5',
                      margin: '20px',
                      transform: visibleCards[card.id] ? 'translateY(0)' : 'translateY(100%)',
                  ...
                  />
                  {/* Text Animation: Move from top */}
                  <CardContent sx={{
                    width: { xs: '100%', sm: '50%' , lg:'45%'}, // Full width on smaller screens, 50% on larger screens
                    paddingTop: 2,
                    display: 'flex',
                  ...
                  >
                    <Typography variant="h1" component="h2" sx={{ fontWeight: '700', margin: 1,color:'#C0029D',background:'#f5f2f5' }}>
                      {card.title}
                    </Typography>
                  </CardContent>
                </Card>
